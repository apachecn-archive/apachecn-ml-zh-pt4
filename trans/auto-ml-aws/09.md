...
22.  在构建阶段，我们定义了一个调用`workflow_build` CodeBuild 项目的动作。您会记得这是创建和执行Step Functions 状态机的 CodeBuild 项目。下面的代码片段声明了引用`workflow_build`项目的`BuildModel`阶段动作:
