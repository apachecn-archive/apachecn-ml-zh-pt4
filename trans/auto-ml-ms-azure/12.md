...
幸运的是，AMLS 有专门为再培训模型构建的专门的 ML 管道步骤。在本节中，我们将使用其中的一个步骤，AutoML 步骤。 **AutoML 步骤**可让您随时轻松地重新培训模型，只需按下按钮或按计划进行。

在这里，您将构建一个两步 ML 管道，首先使用 AutoML 步骤训练一个模型，然后使用典型的 Python 脚本步骤注册它。这将使您能够通过自动化评分和培训构建完整的端到端解决方案，完善您的技能组合。此外，这将使您熟悉 AutoML 步骤及其所有注意事项。

重要说明

并非所有的 ML 模型都需要重新训练，尤其是那些预测物理现象的模型，因为数据和目标变量之间的关系不太可能随时间而改变。然而，大多数 ML 模型将随着额外的数据点而改进，因此当您收集和标注更多数据时，重新训练模型是有意义的。

到目前为止，您应该知道在创建 ML 管道方面会发生什么。大多数步骤您都很熟悉，但是您将不得不使用专门形式的管道数据，这些数据将数据从培训步骤传递到模型注册步骤。

此管道中涉及的 Python 脚本比评分管道中涉及的脚本简单得多，因此当您使用自己的数据尝试时，需要的定制较少。在本节的最后，像其他人一样，您也将向端点发布您的 AutoML 培训管道。*图 9.8* 概述了该过程:

![Figure 9.8 – Steps involved in setting up an AutoML retraining pipeline

](img/Figure_9.8_B16595.jpg)

图 9.8–建立 AutoML 再培训管道的步骤

这个过程与其他管道略有不同，因为您需要在早期配置 AutoML 设置。您可以在 GitHub 存储库中的`ML-Retraining-Pipeline.ipynb`文件中找到所有步骤的代码。

## 编写 AutoML 培训流程的前两步

要创建您的 AutoML 培训渠道，请从以下步骤开始:
...
这将仅用于您的型号注册步骤；您的 AutoML 培训步骤将使用标准的自动生成环境。
...
配置好 AutoML 培训步骤后，您现在需要编写另一个脚本来提取并注册您的模型，步骤如下:
...
现在，您已经构建了三个 ML 管道，一个标准评分管道，一个并行运行管道和一个 AutoML 培训管道。这不是一个微不足道的努力，但所有的努力都得到了回报。你已经掌握了 AMLS 最复杂的部分之一。
...
3.  要手动触发 ML 管道，请使用下面的代码，将`your-published-pipeline-id`替换为您发布的 AutoML 培训管道的 ID。就是这样:
...
您现在已经创建了一个每天自动触发一次 AutoML 培训渠道的计划。该计划将自动启动您的计算集群，训练模型，然后停止运行。许多公司花费数年时间试图找出如何及时、可靠地安排 ML 培训和评分运行的最佳方式，而你仅用一章就完成了这项任务！
