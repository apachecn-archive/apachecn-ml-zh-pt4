...
所有机器学习(ML)活动的目标都是将原始数据转化为某种预测、分类或洞察力。原始数据出现在这条流水线的左边或开头，右边或结尾是洞察/预测/分类等等。虽然每个机器学习任务将需要不同的流水线，但基本结构或构建块保持不变。ML.NET 提供了几种类型/接口来简化流水线的创建。对这些概念的广泛理解将有助于你理解 ML.NET 是如何工作的。
