...
在石灰的当前实现中，取样可以改进。数据点是从高斯分布中采样的，忽略了特征之间的相关性。这可能导致不太可能的数据点，这些数据点然后可以用于学习局部解释模型。
...
这些图有助于提取见解，并检查您的模型是否正在学习您认为合理的东西。
...
结果是一组点，这些点具有来自格网的特征值和相应的预测值。看个体期望而不看部分依赖有什么意义？部分依赖图可能会掩盖由交互创建的异构关系。PDP 可以向您展示特征和预测之间的平均关系。只有在计算 PDP 的特征和其他特征之间的相互作用很弱的情况下，这种方法才有效。在交互中，ICE 图提供了更多的见解。
...
1.  从 numerical _ transformer 流水线开始。以下说明了数字特征。
    1.  选择要变换的数字要素。
...
2.  接下来，categorical _ transformer 流水线。以下说明了 categorical _ features。
    1.  选择要变换的分类要素。
...
3.  使用 ColumnTransformer 将这两个流水线聚合到一个预处理器中。
